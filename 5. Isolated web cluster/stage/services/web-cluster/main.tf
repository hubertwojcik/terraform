...
resource "aws_launch_template" "example" {
    ...
    # Skrypt uruchamiany przy starcie instancji - templatefile wstawia zmienne do szablonu user-data.sh
    # Skrypt tworzy index.html z informacjami o bazie danych (adres i port) i uruchamia serwer HTTP
    # Dane o bazie danych są pobierane z remote state (S3) modułu data-storage/my-sql
...
# Pobranie stanu z modułu bazy danych przechowywanego w S3
# Pozwala na dostęp do outputs z modułu data-storage/my-sql (adres i port bazy danych)
# Stan jest przechowywany w S3 bucket określonym przez zmienne db_remote_state_bucket i db_remote_state_key
data "terraform_remote_state" "db" {
    backend = "s3"
    config = {
        bucket = var.db_remote_state_bucket  # Nazwa S3 bucket z remote state
        key = var.db_remote_state_key  # Ścieżka do pliku state w bucket (np. "stage/data-storage/my-sql/terraform.tfstate")
        region = "eu-north-1"
    }
}